  ...
  return (
    ...
    <div name='home' className={props.isNav ? 'text-white blur-sm' : 'text-white'}>
      <div className='max-w-[800px] mt-[-70px] w-full h-auto sm:h-screen mx-auto text-left flex flex-col justify-center px-4'>
        ...
        {!readMore ?
          aboutMe.substring(0, 160)
          :aboutMe
        }
          <span onClick={()=> setReadMore(!readMore)} className="cursor-pointer text-sm font-extrabold text-gray-400 hover:text-[#d19a66]">{ !readMore ?` read more..` : ` hide` }</span>
        </p>
